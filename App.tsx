...
import { MEMORIES as INITIAL_MEMORIES, LETTERS as INITIAL_LETTERS } from './constants';
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4">
      <main className="flex-1 flex items-center justify-center w-full max-w-7xl">
        {view === 'PROPOSAL' ? (
      ...
      <footer className="w-full py-6 text-center text-rose-300 text-sm font-medium">
        Built with 💖 by <a href="#" className="hover:text-rose-500 underline transition-colors">Your Name</a>
      </footer>
      ...
      {isEditMode && (
        <div className="fixed bottom-6 left-6 bg-white/90 backdrop-blur-md p-4 rounded-3xl shadow-xl border border-rose-100 z-50 animate-bounce">
          <p className="font-bold text-rose-600 text-sm mb-1">✨ Creator Mode Active</p>
          <p className="text-rose-400 text-xs">Share this URL without <b>?mode=edit</b> to your Valentine!</p>
        </div>
      )}
    </div>
  );
